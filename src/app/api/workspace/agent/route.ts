...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      log("warn", "[GET /api/workspace/agent] business context query failed");
    }
  ...
  try {
    ...
    if (Object.keys(businessContextUpdate).length > 0) {
      ...
      try {
        ...
        log("warn", "[workspace/agent] workspace_business_context update failed:",
          { detail: ctx_err instanceof Error ? ctx_err.message : String(ctx_err) });
      }
    }
