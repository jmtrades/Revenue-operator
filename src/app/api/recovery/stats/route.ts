/**
 * GET /api/recovery/stats — Aggregated missed-call recovery statistics.
 * Combines call_sessions (missed call outcomes) with daily_metrics for recovery data.
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Count total missed calls (no_answer, busy, voicemail)
    ...
    // Count recovered (missed calls with metadata.recovered = true)
    ...
    console.error("[recovery/stats] unexpected error:", e);
    return NextResponse.json({
      total_missed: 0,
      recovered: 0,
      pending: 0,
