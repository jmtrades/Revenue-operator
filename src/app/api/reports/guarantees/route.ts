/**
 * Guarantees Panel: response SLA, follow-up SLA, booking accuracy, safety compliance
...
export async function GET(req: NextRequest) {
  ...
  const responseSla = total > 0 ? (under60 / total) * 100 : 100;
  ...
  const s = scheduled ?? 0;
  const e = executed ?? 0;
  const followupSla = s > 0 ? Math.min(100, (e / s) * 100) : 100;

  const { data: deals } = await db
    .from("deals")
    .select("id, status")
    .eq("workspace_id", workspaceId)
    .in("status", ["booked", "showed", "won"])
    .gte("created_at", weekStart.toISOString());

  const booked = (deals ?? []).filter((d) => (d as { status: string }).status === "booked").length;
  ...
  const safetyCompliance = 100;

  return NextResponse.json({
    response_sla_pct: Math.round(responseSla * 100) / 100,
    followup_sla_pct: Math.round(followupSla * 100) / 100,
    booking_accuracy_pct: Math.round(bookingAccuracy * 100) / 100,
    safety_compliance_pct: safetyCompliance,
