...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    result.users = {
      ...
      recent: (recentUsers ?? []).map((u: Record<string, any>) => ({
        id: u.id,
  ...
  try {
    ...
    (allWs ?? []).forEach((w: Record<string, any>) => {
    ...
    result.workspaces = {
      ...
      recent: (recentWorkspaces ?? []).map((w: Record<string, any>) => ({
        id: w.id,
        name: w.name,
  ...
  try {
    ...
    (growth ?? []).forEach((u: Record<string, any>) => {
  ...
  let voiceServerHealth: Record<string, any> | null = null;
  let voiceServerStatus: Record<string, any> | null = null;
  ...
  try {
    ...
    if (voiceUrl) {
      ...
      if (resp?.ok) {
        ...
        voiceServerHealth = (await resp.json().catch(() => null)) as Record<string, any> | null;

        const statusResp = await fetch(`${voiceUrl}/status`, { method: "GET", cache: "no-store" }).catch(() => null);
        if (statusResp?.ok) voiceServerStatus = (await statusResp.json().catch(() => null)) as Record<string, any> | null;
      }
    }
  } catch {
    voiceServerOk = false;
