...
export async function GET(request: NextRequest) {
  ...
  await processWebhookDeliveries().catch((e: unknown) => {
      log("warn", "[cron/process-queue] webhook deliveries failed:", { detail: e instanceof Error ? e.message : String(e) });
    });
  ...
  if (job) {
    ...
    await recordCronHeartbeat("process-queue").catch((e: unknown) => {
      log("warn", "[cron/process-queue] heartbeat failed:", { detail: e instanceof Error ? e.message : String(e) });
    });
