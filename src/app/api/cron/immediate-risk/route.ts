...
export async function GET(request: NextRequest) {
  ...
  const result = await runSafeCron("immediate-risk", async () => {
    ...
    await runContinuationPreventedCheck().catch((e: unknown) => { log("warn", "non-blocking-catch", { error: String(e) }); });
    await runPropagationIgnition().catch((e: unknown) => { log("warn", "non-blocking-catch", { error: String(e) }); });
    return { run: 1 };
  });
