...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (auditInsertErr) {
      log("warn", "[change-plan] Failed to log billing event", { error: auditInsertErr.message });
    }
  } catch (auditErr) {
    // Non-blocking: audit log error should not fail the plan change
    log("warn", "[change-plan] Audit log error", { error: auditErr instanceof Error ? auditErr.message : String(auditErr) });
  }
