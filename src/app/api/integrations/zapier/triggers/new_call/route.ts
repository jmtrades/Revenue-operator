...
export async function GET(req: NextRequest) {
  ...
  if (since) {
    const { data: cursorRow } = await db.from("call_sessions").select("call_started_at").eq("id", since).eq("workspace_id", workspaceId).maybeSingle();
    if (cursorRow) {
      const ts = (cursorRow as { call_started_at: string }).call_started_at;
      query = query.lt("call_started_at", ts);
    }
  }
