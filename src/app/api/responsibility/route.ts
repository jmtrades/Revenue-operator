...
export async function GET(request: NextRequest) {
  ...
  }).catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); });
  await advanceObservationPhaseIfDue(workspaceId).catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); });

  for (const e of incomingEntries) {
    import("@/lib/record-reference").then(({ recordRecordReference }) =>
      recordRecordReference(workspaceId, "staff", "dashboard_record", e.external_ref).catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); })
    );
  }
  ...
  recordAssumptionOrientationOnce(workspaceId).catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); });
  ...
  if (provider_detached) {
    ...
    if (!alreadyRecorded) {
      ...
      await recordOrientationStatement(workspaceId, "The provider was no longer required for operation.").catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); });
  ...
  if (operation_currently_anchored) {
    ...
    if (!alreadyRecorded) {
      const { recordOrientationStatement } = await import("@/lib/orientation/records");
      await recordOrientationStatement(workspaceId, "The operation became sustained through the process.").catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); });
      const now = new Date().toISOString();
      try {
        const { data: row } = await db.from("workspace_orientation_state").select("workspace_id").eq("workspace_id", workspaceId).maybeSingle();
        if (row) {
  ...
  if (structurally_dependent) {
    ...
    if (!alreadyRecorded) {
      const { recordOrientationStatement } = await import("@/lib/orientation/records");
      await recordOrientationStatement(workspaceId, "The organization operated through the environment.").catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); });
      const now = new Date().toISOString();
      try {
        if (orientRow) {
          await db
  ...
  if (operationally_embedded) {
    ...
    if (!alreadyRecorded) {
      const { recordOrientationStatement } = await import("@/lib/orientation/records");
      await recordOrientationStatement(workspaceId, "The operating process became established.").catch((err) => { log("error", "[responsibility] error:", { error: err instanceof Error ? err.message : err }); });
      const now = new Date().toISOString();
      try {
        if (orientRow) {
          await db
