                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.feature} className="border-b border-[var(--border-default)]">
                      ...
                      <td className="px-6 py-4 text-center">
                        {r.ruby ? <CheckCircle2 className="w-5 h-5 mx-auto text-emerald-400" aria-hidden /> : <XCircle className="w-5 h-5 mx-auto text-zinc-600" aria-hidden />}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <section className="mt-12 max-w-5xl mx-auto">
            <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-inset)] p-8">
              ...
              <div className="mt-6 flex flex-col sm:flex-row gap-3">
                ...
                <Link href="/results" className="btn-marketing-secondary inline-flex items-center justify-center px-6 py-3 rounded-xl font-semibold no-underline">
                  See real results →
                </Link>
              </div>
              <p className="mt-4 text-xs" style={{ color: "var(--text-tertiary)" }}>
                Tip: start by recovering missed calls and no-shows, then layer outbound sequences when your inbound volume is already stable.
              </p>
            </div>
          </section>
        </Container>
