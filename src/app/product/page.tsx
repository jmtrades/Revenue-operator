        ...
        <Container>
          <section className="max-w-3xl mb-16">
            ...
            <p className="text-lg" style={{ color: "var(--text-secondary)", lineHeight: 1.65 }}>
              Answer calls, book appointments, follow up with leads, and keep every conversation documented — for any business or team.
            </p>
          </section>

          <section className="mb-20">
            <h2 className="font-semibold text-xl mb-8" style={{ color: "var(--text-primary)" }}>Core capabilities</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {PRODUCT_SECTIONS.map((s) => (
                <div key={s.id} id={s.id} className="card-marketing p-5 flex flex-col">
                  ...
                  <p className="text-sm flex-1" style={{ color: "var(--text-secondary)", lineHeight: 1.55 }}>{s.desc}</p>
                </div>
              ))}
            </div>
          </section>

          <section id="compliance-governance" className="mb-20 py-12 px-6 rounded-2xl border" style={{ borderColor: "var(--border-default)", background: "var(--bg-surface)" }}>
            <h2 className="font-semibold text-xl mb-4" style={{ color: "var(--text-primary)" }}>Compliance & governance</h2>
            <p className="text-base mb-4" style={{ color: "var(--text-secondary)", lineHeight: 1.65 }}>
              Every call is recorded and transcribed. Governed records, audit trail, and chain of custody. HIPAA mode available. Retention 30–365 days. Data export and industry-ready documentation. When your industry requires it, Recall Touch is built for it.
            </p>
            <ul className="space-y-1.5 text-sm" style={{ color: "var(--text-secondary)" }}>
              <li>Recording & transcription · HIPAA option · Retention 30–365 days · Audit trail</li>
            </ul>
          </section>

          <section className="mt-16 pt-12 border-t" style={{ borderColor: "var(--border-default)" }}>
            <h2 className="font-semibold text-xl mb-8 text-center" style={{ color: "var(--text-primary)" }}>Use cases</h2>
            <div className="grid md:grid-cols-3 gap-6">
              <div className="card-marketing p-6 flex flex-col">
                <h3 className="font-semibold text-lg mb-2" style={{ color: "var(--text-primary)" }}>For businesses</h3>
                <p className="text-sm flex-1" style={{ color: "var(--text-secondary)", lineHeight: 1.65 }}>Solo operators to growing teams. Answer every call, book appointments, capture leads, and follow up — without hiring a front desk.</p>
                <Link href={ROUTES.START} className="text-sm font-medium mt-4 inline-block" style={{ color: "var(--accent-primary)" }}>Get started →</Link>
              </div>
              <div className="card-marketing p-6 flex flex-col">
                <h3 className="font-semibold text-lg mb-2" style={{ color: "var(--text-primary)" }}>For teams</h3>
                <p className="text-sm flex-1" style={{ color: "var(--text-secondary)", lineHeight: 1.65 }}>Multi-location, agencies, and distributed teams. One platform, one standard. Consistent answers and full visibility.</p>
                <Link href={ROUTES.START} className="text-sm font-medium mt-4 inline-block" style={{ color: "var(--accent-primary)" }}>Get started →</Link>
              </div>
              <div className="card-marketing p-6 flex flex-col">
                <h3 className="font-semibold text-lg mb-2" style={{ color: "var(--text-primary)" }}>For developers</h3>
                <p className="text-sm flex-1" style={{ color: "var(--text-secondary)", lineHeight: 1.65 }}>API, webhooks, and integrations. Build Recall Touch into your stack and keep every conversation documented.</p>
                <Link href={ROUTES.DOCS} className="text-sm font-medium mt-4 inline-block" style={{ color: "var(--accent-primary)" }}>Documentation →</Link>
              </div>
            </div>
          </section>
