      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-8 gap-6" style={{ background: "var(--background)" }}>
        ...
        <button
          ...
          onClick={() => {
            ...
            if (url) navigator.clipboard.writeText(url).catch((e: unknown) => { console.warn("[page] failed:", e instanceof Error ? e.message : String(e)); });
          }}
        ...
        <div className="mt-8">
          {isRecordAction ? (
            <section className="rounded-[12px] border p-6" style={{ borderColor: "var(--border)" }}>
              <button
                type="button"
                onClick={() => { if (canonicalRecordUrl) navigator.clipboard.writeText(canonicalRecordUrl).catch((e: unknown) => { console.warn("[page] failed:", e instanceof Error ? e.message : String(e)); }); }}
