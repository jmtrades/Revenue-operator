    ...
    <div className="min-h-screen bg-[var(--bg-primary)] p-6 space-y-6">
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-lg p-6">
        <div className="flex gap-6 items-start">
          {/* Avatar placeholder */}
          <div className="w-24 h-24 bg-[var(--bg-hover)] rounded-full skeleton-shimmer shrink-0" />

          <div className="flex-1 space-y-4">
            {/* Name skeleton */}
            <div className="space-y-2">
              <div className="h-8 bg-[var(--bg-hover)] rounded w-48 skeleton-shimmer" />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="space-y-2">
                  <div className="h-3 bg-[var(--bg-hover)] rounded w-16 skeleton-shimmer" />
      ...
      <div className="flex gap-4 border-b border-[var(--border-default)]">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-10 bg-[var(--bg-hover)] rounded w-24 skeleton-shimmer" />
        ))}
      </div>
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-lg p-6 space-y-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex gap-4 pb-4 border-b border-[var(--border-default)] last:border-0">
            <div className="w-10 h-10 bg-[var(--bg-hover)] rounded-lg shrink-0 skeleton-shimmer" />
            <div className="flex-1 space-y-2">
              <div className="h-4 bg-[var(--bg-hover)] rounded w-32 skeleton-shimmer" />
              <div className="h-4 bg-[var(--bg-hover)] rounded w-48 skeleton-shimmer" />
              <div className="h-3 bg-[var(--bg-hover)] rounded w-20 skeleton-shimmer" />
            </div>
          </div>
        ))}
      </div>
