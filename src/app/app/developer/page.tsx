          ...
          <div key={wh.id} className="rounded-xl border border-[var(--border-default)] overflow-hidden bg-[var(--bg-card)]/30">
            ...
            {expandedId === wh.id && (
              <div className="border-t border-[var(--border-default)] p-4 bg-[var(--bg-card)]">
                <p className="text-xs font-medium text-zinc-500 mb-3">{t("recentDeliveries")}</p>
                <div className="space-y-2">
      ...
      {addModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 overflow-y-auto" onClick={(e) => e.target === e.currentTarget && setAddModal(false)}>
          <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-2xl w-full max-w-md p-6 my-8" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-white mb-4">{t("addWebhookTitle")}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">{t("urlLabel")}</label>
                <input
                  type="url"
                  value={newUrl}
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">{t("secretAutoGenerated")}</label>
                <div className="flex items-center gap-2">
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-2">{t("eventsLabel")}</label>
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button type="button" onClick={() => setAddModal(false)} className="px-4 py-2 rounded-xl text-sm text-zinc-400 border border-[var(--border-medium)]">{t("cancel")}</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70" onClick={(e) => e.target === e.currentTarget && setPayloadModal(null)}>
          <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[var(--border-default)]">
              <h3 className="text-sm font-semibold text-white">{t("payload")}</h3>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        <span className="text-xs text-zinc-500 self-center mr-2">{t("eventTypeLabel")}</span>
        ...
        <span className="text-xs text-zinc-500 self-center ml-4 mr-2">{t("statusLabel")}</span>
      ...
      <div className="hidden md:block rounded-xl border border-[var(--border-default)] overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-[var(--bg-card)]/80 border-b border-[var(--border-default)]">
            <tr>
              <th className="py-3 px-4 font-medium text-zinc-400">{t("time")}</th>
              <th className="py-3 px-4 font-medium text-zinc-400">{t("event")}</th>
              <th className="py-3 px-4 font-medium text-zinc-400">{t("endpointUrl")}</th>
              <th className="py-3 px-4 font-medium text-zinc-400">{t("status")}</th>
              <th className="py-3 px-4 font-medium text-zinc-400">{t("responseTime")}</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((row) => (
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="p-4 md:p-6 lg:p-8">
        <h1 className="text-xl md:text-2xl font-semibold text-white mb-2">{t("heading")}</h1>
          ...
          {tabs.map((tabDef) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 -mb-px transition-colors ${
                tab === tabDef.id ? "border-white text-white" : "border-transparent text-zinc-500 hover:text-zinc-300"
              }`}
            >
