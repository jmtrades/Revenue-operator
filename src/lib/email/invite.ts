...
const FROM = process.env.EMAIL_FROM ?? "Recall Touch <team@recall-touch.com>";
...
<body style="font-family: system-ui, sans-serif; line-height: 1.6; color: #333; max-width: 560px; margin: 0 auto; padding: 24px;">
  <p>Hi there,</p>
  <p><strong>${escapeHtml(inviterName)}</strong> invited you to join <strong>${escapeHtml(workspaceName)}</strong> as a <strong>${escapeHtml(role)}</strong> on Recall Touch.</p>
  <p>Recall Touch is an AI calling platform that handles phone calls, books appointments, and captures leads automatically.</p>
  ...
  <p style="font-size: 13px; color: #666;">— The Recall Touch team</p>
</body>
</html>
`.trim();
}
...
export async function sendInviteEmail(to: string, params: InviteEmailParams): Promise<{ ok: boolean; error?: string }> {
  ...
  const subject = `${params.inviterName} invited you to ${params.workspaceName} on Recall Touch`;
